...
from operation_hole import operation, force_sense, Drill_on
...
pose1 = CreateHole("Hole 1", aruco_pose , -70, 80, 0) # -70, 35, 0
...
def move_hole(pose, char):
    ...
    robot.MoveL(robot.SolveFK(robot.Joints())*ENDEFFECTOR_TO_DRILL*transl(31,74,27)) # Move Drill to infront of the Hole, Move to x,y position of the Camera, move forward in the z
    input("enter")
    ...
    speed() # Re-set Speed
    input("enter")
    robot.MoveL(robot.SolveFK(robot.Joints())*ENDEFFECTOR_TO_DRILL*transl(0,0,-32)) # Move the Drill out until the Debarring tip is around the walls of the Hole (REAR)
    input("enter")
    Drill_on() # Strat Debarring
    connect_DK() # Re-Connect with the Robot
    speed() # Re-set Speed
    input("enter")
    robot.MoveL(robot.SolveFK(robot.Joints())*ENDEFFECTOR_TO_DRILL*transl(0,0,-8)) # Move the Drill out until the Debarring tip is around the walls of the Hole (FRONT)
    input("enter")
    Drill_on() # Strat Debarring
    connect_DK() # Re-Connect with the Robot
    speed() # Re-set Speed
    ...
    robot.MoveL(robot.SolveFK(robot.Joints())*ENDEFFECTOR_TO_DRILL*transl(-31,-74,-27)) # Put Camera infront of the Hole again
