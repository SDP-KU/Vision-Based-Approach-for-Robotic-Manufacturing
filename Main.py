...
C2E_y = -0.0373*1000
C2E_z = 0.1996*1000
C2E_rx = np.multiply(-1.5702158, 180/math.pi)
C2E_ry = np.multiply(0.0320472, 180/math.pi)
C2E_rz = np.multiply(-2.3870739, 180/math.pi)

# Drill position with respect to end-effector
D2E_x = 0
D2E_y = 0
D2E_z = 150
D2E_rx = np.multiply(-1.5702158, 180/math.pi)
D2E_ry = np.multiply(0.0320472, 180/math.pi)
D2E_rz = np.multiply(-2.3870739, 180/math.pi)
